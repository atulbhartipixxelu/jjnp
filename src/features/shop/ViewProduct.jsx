...
export default function ViewProduct(props) {
  ...
  const handleAddToCart = async () => {
    if (!selectedSize || !selectedColor || selectedQty < 1) {
      setToastMessage("Please select size, color, and quantity.");
      return;
    }
      ...
      <div className="relative">
        ...
        <div className="absolute inset-x-0 inset-y-0 hidden flex-wrap items-center justify-center lg:flex  ">
          <div className="z-50 w-[250px] h-[150px] flex items-center justify-center flex-wrap bg-white shadow-md">
            <h3 className="text-black text-center text-[24px] font-bold w-full">{product.title} <span className="mt-0 lg:mt-1 font-normal block text-gray-700 text-[18px] w-full price">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'INR' }).format(product.originalPrice)}</span><span className="text-black text-center uppercase text-[16px] font-bold w-full">Available</span>
            </h3>
